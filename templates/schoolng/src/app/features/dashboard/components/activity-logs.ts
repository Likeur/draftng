...
@Component({
  ...
  template: `
    <div class="rounded-xl border bg-theme-panel border-theme-border p-5 space-y-3">
      <div>
        <h3 class="font-medium text-xs text-theme-text-main tracking-wider capitalize">Registrar Activity</h3>
        <p class="text-[10px] text-theme-text-muted font-normal mt-1">Live audit log of registrar activity and marks entries</p>
      </div>

      <div class="divide-y divide-theme-border font-sans">
        @for (log of filteredLogs(); track log.id) {
          <div class="py-2.5 flex items-start justify-between gap-4 text-xs font-normal animate-blur-slide">
            <div class="flex items-start gap-3">
              ...
              <p class="text-theme-text-main leading-normal">{{ log.message }}</p>
            </div>
            <span class="text-[9px] text-theme-text-muted whitespace-nowrap pt-0.5 font-mono select-none font-normal">{{ log.time }}</span>
          </div>
        }
      </div>
    </div>
