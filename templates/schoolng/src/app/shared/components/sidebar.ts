      ...
      <div>
        ...
        <div class="mb-5">
          ...
          <nav class="space-y-0.5">
            ...
            <button 
              routerLink="/classes"
              [class]="isActive('/classes') ? 'bg-theme-hover text-theme-text-main font-medium' : 'text-theme-text-muted hover:text-theme-text-main hover:bg-theme-hover'" 
              class="w-full flex items-center py-2 rounded-lg text-xs transition-all cursor-pointer text-left overflow-hidden font-normal clickable-scale"
              [class.justify-center]="state.isCollapsed()"
              [class.gap-3]="!state.isCollapsed()"
              [class.px-2]="state.isCollapsed()"
            ...
            <button 
              routerLink="/students"
              [class]="isActive('/students') ? 'bg-theme-hover text-theme-text-main font-medium' : 'text-theme-text-muted hover:text-theme-text-main hover:bg-theme-hover'" 
              class="w-full flex items-center py-2 rounded-lg text-xs transition-all cursor-pointer text-left overflow-hidden font-normal clickable-scale"
              [class.justify-center]="state.isCollapsed()"
              [class.gap-3]="!state.isCollapsed()"
              [class.px-2]="state.isCollapsed()"
            ...
            <button 
              routerLink="/teachers"
              [class]="isActive('/teachers') ? 'bg-theme-hover text-theme-text-main font-medium' : 'text-theme-text-muted hover:text-theme-text-main hover:bg-theme-hover'" 
              class="w-full flex items-center py-2 rounded-lg text-xs transition-all cursor-pointer text-left overflow-hidden font-normal clickable-scale"
              [class.justify-center]="state.isCollapsed()"
              [class.gap-3]="!state.isCollapsed()"
              [class.px-2]="state.isCollapsed()"
            ...
            <button 
              routerLink="/settings"
              [class]="isActive('/settings') ? 'bg-theme-hover text-theme-text-main font-medium' : 'text-theme-text-muted hover:text-theme-text-main hover:bg-theme-hover'" 
              class="w-full flex items-center py-2 rounded-lg text-xs transition-all cursor-pointer text-left overflow-hidden font-normal clickable-scale"
              [class.justify-center]="state.isCollapsed()"
              [class.gap-3]="!state.isCollapsed()"
              [class.px-2]="state.isCollapsed()"
          ...
          <div class="space-y-0.5 px-1">
            @for (cohort of cohorts; track cohort.name) {
              <button class="w-full flex items-center py-1.5 rounded-lg text-xs text-theme-text-muted hover:text-theme-text-main hover:bg-theme-hover transition-all text-left overflow-hidden clickable-scale"
                ...
                [class.px-2]="state.isCollapsed()"
                [class.px-3]="!state.isCollapsed()">
                <span [class]="cohort.color" class="w-2 h-2 rounded-full shrink-0 border border-theme-border"></span>
                @if (!state.isCollapsed()) {
                  <span class="animate-fade-in truncate font-normal text-theme-text-muted">{{ cohort.name }}</span>
                }
              </button>
            }
          </div>
        ...
        <div 
          ...
          <div class="flex items-center gap-2.5 p-2.5 pb-3 border-b border-theme-border">
            ...
            <div class="text-left overflow-hidden">
              <p class="text-xs font-medium leading-tight text-theme-text-main truncate">admin_schoolng</p>
              <p class="text-[9px] text-theme-text-muted truncate mt-0.5">registrar&#64;schoolng.edu</p>
            </div>
          </div>
          ...
          <div class="py-1">
            <!-- Settings Link -->
            <button routerLink="/settings" (click)="isUserDropdownOpen.set(false)" class="w-full flex items-center justify-between px-2.5 py-2 rounded-lg text-xs text-theme-text-muted hover:text-theme-text-main transition-all cursor-pointer text-left clickable-scale">
              <div class="flex items-center gap-2.5">
                <!-- Lucide: settings -->
                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="shrink-0"><path d="M12.22 2h-.44a2 2 0 0 0-2 2v.18a2 2 0 0 1-1 1.73l-.43.25a2 2 0 0 1-2 0l-.15-.08a2 2 0 0 0-2.73.73l-.22.38a2 2 0 0 0 .73 2.73l.15.1a2 2 0 0 1 1 1.72v.51a2 2 0 0 1-1 1.74l-.15.09a2 2 0 0 0-.73 2.73l.22.38a2 2 0 0 0 2.73.73l.15-.08a2 2 0 0 1 2 0l.43.25a2 2 0 0 1 1 1.73V20a2 2 0 0 0 2 2h.44a2 2 0 0 0 2-2v-.18a2 2 0 0 1 1-1.73l.43-.25a2 2 0 0 1 2 0l.15.08a2 2 0 0 0 2.73-.73l.22-.39a2 2 0 0 0-.73-2.73l-.15-.08a2 2 0 0 1-1-1.74v-.5a2 2 0 0 1 1-1.74l.15-.1a2 2 0 0 0 .73-2.73l-.22-.38a2 2 0 0 0-2.73-.73l-.15.08a2 2 0 0 1-2 0l-.43-.25a2 2 0 0 1-1-1.73V4a2 2 0 0 0-2-2z"/><circle cx="12" cy="12" r="3"/></svg>
                <span>Global Settings</span>
              </div>
              <span class="text-[9px] text-theme-text-muted font-mono tracking-wider">⇧⌘,</span>
            </button>
          </div>
          ...
          <div class="flex items-center overflow-hidden shrink-0" [class.gap-2]="!state.isCollapsed()">
            ...
            @if (!state.isCollapsed()) {
              <div class="text-left animate-fade-in shrink-0">
                <p class="text-[10px] font-medium leading-tight text-theme-text-main truncate max-w-30">admin_schoolng</p>
                <p class="text-[8px] text-theme-text-muted font-normal truncate max-w-30">registrar&#64;schoolng.edu</p>
              </div>
            }
          </div>
          @if (!state.isCollapsed()) {
